...
extension List: MutableCollection {
    ...
    //// :nodoc:
    public func removeSubrange1(_ bounds: CountableRange<Int>) {
        for _ in bounds {
    ...
    /// :nodoc:
    public func removeSubrange2(_ bounds: CountableClosedRange<Int>) {
        for _ in bounds {
            remove(at: bounds.lowerBound)
        }
    }
    ...
    /// :nodoc:
    public func replaceSubrange1<C: Collection>(_ subrange: CountableRange<Int>, with newElements: C)
        where C.Iterator.Element == Element {
            removeSubrange(subrange)
            insert(contentsOf: newElements, at: subrange.lowerBound)
    }
    ...
    /// :nodoc:
    public func replaceSubrange2<C: Collection>(_ subrange: CountableClosedRange<Int>, with newElements: C)
        where C.Iterator.Element == Element {
            removeSubrange(subrange)
            insert(contentsOf: newElements, at: subrange.lowerBound)
    }
